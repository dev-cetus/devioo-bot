        ...
        switch (interaction.options.getSubcommand()) {
            case 'join':
                   if (thread.joinable) {
                       thread.join().then(() => {
                           ...
                           interaction.reply({content: '**❌ | Une erreur est survenue lors de la tentative de rejoindre le thread.**', ephemeral: true});
                       });
                   } else {
            ...
            case 'leave':
                thread.leave().then(() => {
                    ...
                    interaction.reply({content: '**❌ | Une erreur est survenue lors de la tentative de quitter le thread.**', ephemeral: true});
                });
                break;
            case 'archive':
                await interaction.reply({content: '**✅ | Le thread a été archivé.**', ephemeral: true});
                await thread.setArchived(true).catch(() => {
                    interaction.reply({content: '**❌ | Une erreur est survenue lors de l\'archivage du thread.**', ephemeral: true});
                });
                break;
            case 'unarchive':
                thread.setArchived(false).then(() => {
                    ...
                    interaction.reply({content: '**❌ | Une erreur est survenue lors de la désarchivage du thread.**', ephemeral: true});
                });
                break;
            case 'delete':
                await thread.delete();
